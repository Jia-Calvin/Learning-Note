...
class ThreadSafeHashTableWithBucketLock {
 public:
  ThreadSafeHashTableWithBucketLock(
      const ThreadSafeHashTableWithBucketLock &other) = delete;
  ThreadSafeHashTableWithBucketLock &operator=(
      const ThreadSafeHashTableWithBucketLock &other) = delete;

  ThreadSafeHashTableWithBucketLock(const int bucketSize = 19,
  ...
  struct Bucket {
    ...
    typename std::list<std::pair<K, V>>::iterator findEntry(const K &key) {
      return std::find_if(
          _bucketList.begin(), _bucketList.end(),
...
ThreadSafeHashTableWithBucketLock<
    K, V, Hash>::ThreadSafeHashTableWithBucketLock(const int bucketSize,
                                                   const Hash &hasher)
    : _hasher(hasher) {
...
template <typename K, typename V, typename Hash>
ThreadSafeHashTableWithBucketLock<K, V,
                                  Hash>::~ThreadSafeHashTableWithBucketLock() {}

template <typename K, typename V, typename Hash>
const int ThreadSafeHashTableWithBucketLock<K, V, Hash>::_getBucketIndex(
    const K key) {
  return _hasher(key) % _buckets.size();
}

template <typename K, typename V, typename Hash>
void ThreadSafeHashTableWithBucketLock<K, V, Hash>::put(const K key,
                                                        const V val) {
  ...
  typename std::list<std::pair<K, V>>::iterator it =
      _buckets[bucketIndex]->findEntry(key);
