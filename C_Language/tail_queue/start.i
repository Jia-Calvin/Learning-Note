...
# 3 "start.c" 2
# 1 "/Library/Developer/CommandLineTools/SDKs/MacOSX10.14.sdk/usr/include/sys/queue.h" 1 3 4
# 4 "start.c" 2
...
struct tailq_entry
{
    ...
    struct { struct tailq_entry *tqe_next; struct tailq_entry **tqe_prev; }
    entries;
};

int main()
{
    struct tailq_head_name { struct tailq_entry *tqh_first; struct tailq_entry **tqh_last; }
    tailq_head;
    struct tailq_entry *iterm;

    do { (((&tailq_head))->tqh_first) = ((void *)0); (&tailq_head)->tqh_last = &(((&tailq_head))->tqh_first); ; } while (0);
    printf("%p\n\n", &tailq_head);
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        iterm->value = i;
        do { (((iterm))->entries.tqe_next) = ((void *)0); (iterm)->entries.tqe_prev = (&tailq_head)->tqh_last; *(&tailq_head)->tqh_last = (iterm); (&tailq_head)->tqh_last = &(((iterm))->entries.tqe_next); ; ; } while (0);
    }
    ...
    printf("\n");
    printf("%d\n", (*(((struct tailq_head_name *)((first_next_next)->entries.tqe_prev))->tqh_last)) ->value);
