...
proxy_listen(struct context *ctx, struct conn *p)
{
    ...
    if (p->family == AF_UNSPEC) {
        const char *str = getenv("LISTEN_FD");

        if (!str) {
            log_error("environment variable LISTEN_FD isn't set");
            return NC_ERROR;
        }
        p->sd = atoi(str);
        if (p->sd < 0) {
            log_error("invalid LISTEN_FD: %d", p->sd);
            return NC_ERROR;
        }
    } else {
...
proxy_each_init(void *elem, void *data)
{
    ...
    if (pool == pool->ctx->sync_cache_del) {
        p = conn_get(pool, true, pool->redis);
        if (!p) {
            return NC_ENOMEM;
        }
        return NC_OK;
    }else if (pool == pool->ctx->sync_write_buf) {
        p = conn_get(pool, true, pool->redis);
        if (!p) {
            return NC_ENOMEM;
        }
        return NC_OK;
    }else if (pool == pool->ctx->sync_cache_peer) {
        p = conn_get(pool, true, pool->redis);
        if (!p) {
            return NC_ENOMEM;
        }
