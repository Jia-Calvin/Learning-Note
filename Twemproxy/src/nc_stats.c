...
static void
dump_stats_pool(struct array* stats_pool)
{
    log_info("dump_stats_pool");
    uint32_t i, j, k;
    uint32_t nfield = STATS_SERVER_NFIELD;
    struct stats_server *sts;
    struct stats_metric *stm;
    log_info("stats_pool size:%u", array_n(stats_pool));
    for(i = 0; i< array_n(stats_pool); i++){
        struct stats_pool * stp = array_get(stats_pool, i);
        log_info("stats_pool server size:%u", array_n(&stp->server));
        for(j = 0; j < array_n(&stp->server); j++){
            sts = array_get(&stp->server, j);
            for (k = 0; k < nfield; k++) {
                stm = array_get(&sts->metric, k);
                log_info("dump stat info: stm-type:%d, name:%.*s",
                    stm->type, stm->name.len, stm->name.data);
            }
        }
    }
}
...
static void
dump_stats_server(struct context *ctx, struct server* server)
{
    struct stats *st;
    ...
    uint32_t i, nfield = STATS_SERVER_NFIELD;
    for (i = 0; i < nfield; i++) {
        stm = array_get(&sts->metric, i);
        log_info("dump current stat info: stm1-type:%d, name:%.*s",
             stm->type, stm->name.len, stm->name.data);
    }

    stp = array_get(&st->shadow, pidx);
    sts = array_get(&stp->server, sidx);
    for (i = 0; i < nfield; i++) {
        stm = array_get(&sts->metric, i);
        log_info("dump shadow stat info: stm1-type:%d, name:%.*s",
             stm->type, stm->name.len, stm->name.data);
    }

}

void dump_stats_server_pool(struct context *ctx, struct server_pool *pool)
{
    uint32_t i;
    struct array *server = &pool->server;
    for (i = 0; i < array_n(server); i++) {
        struct server *s = array_get(server, i);
        dump_stats_server(ctx, s);
    }
}
...
stats_aggregate_metric(struct array *dst, struct array *src)
{
    ...
    for (i = 0; i < array_n(src); i++) {
        ...
        /**
        * tmp fix for: stm1-type:7686080, name:sync_requests,stm2-type:1, name:sync_requests
        */
        if (stm1->type != stm2->type){
            log_info("Warning!!! stm1-type:%d, name:%.*s, stm2-type:%d, name:%.*s",
            stm1->type, stm1->name.len, stm1->name.data,
            stm2->type, stm2->name.len, stm2->name.data);
            stm1->type = stm2->type;
        }
...
rstatus_t
stats_rebuild(struct context *ctx)
{
    rstatus_t status;

    stats_pool_unmap(&ctx->stats->sum);
    stats_pool_unmap(&ctx->stats->shadow);
    stats_pool_unmap(&ctx->stats->current);
    stats_destroy_buf(ctx->stats);

    status = stats_pool_map(&ctx->stats->current, &ctx->pool);
    if (status != NC_OK) {
        stats_destroy(ctx->stats);
        return status;
    }
    status = stats_pool_map(&ctx->stats->shadow, &ctx->pool);
    if (status != NC_OK) {
        stats_destroy(ctx->stats);
        return status;
    }
    status = stats_pool_map(&ctx->stats->sum, &ctx->pool);
    if (status != NC_OK) {
        stats_destroy(ctx->stats);
        return status;
    }
    status = stats_create_buf(ctx->stats);
    if (status != NC_OK) {
        stats_destroy(ctx->stats);
        return status;
    }
