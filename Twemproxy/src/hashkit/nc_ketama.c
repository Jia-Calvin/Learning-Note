...
ketama_update(struct server_pool *pool)
{
    ...
    uint32_t need_update;         /* is need to update */
    ...
    nlive_server = 0;
    uint32_t fail_server[1000] = {0};
    uint32_t nfail_server = 0;
    total_weight = 0;
    ...
    for (server_index = 0; server_index < nserver; server_index++) {
        ...
        if (pool->auto_eject_hosts) {
            /* failover problem server */
            if(server->failure_count < pool->server_failure_limit) {
                server->next_retry = 0LL;
                nlive_server++;
            } else {
                pool->next_rebuild = now + pool->server_retry_timeout;
                fail_server[nfail_server] = server_index;
                nfail_server++;
            }
        } else {
            nlive_server++;
        ...
        if (!pool->auto_eject_hosts || server->failure_count < pool->server_failure_limit) {
            total_weight += server->weight;
        }
    }

    /* need to update continuum? */
    need_update = 0;
    if (pool->nlive_server == nlive_server) {
        for (server_index = 0; server_index < nfail_server; server_index++) {
            if (pool->fail_server[server_index] != fail_server[server_index]) {
                need_update = 1;
            }
        }
    } else {
    ...
    /* set new nlive_server */
    pool->nlive_server = nlive_server;
    memset(pool->fail_server, 0, sizeof(pool->fail_server));
    for (server_index = 0; server_index < nfail_server;server_index++) {
        pool->fail_server[server_index] = fail_server[server_index];
    }
    if (!need_update) {
        log_warn("no need to update. live server number %"PRIu32"", nlive_server);
        return NC_OK;
    } else {
        log_warn("need to update. old %"PRIu32", new %"PRIu32"", pool->nlive_server, nlive_server);
    }
...
void
dump_continuum(struct continuum * continuum, uint32_t ncontinuum)
{
    struct continuum *begin, *now;
    begin = continuum;
    size_t i;
    for(i =0 ; i < ncontinuum; i++){
        now = begin +i;
        log_debug(LOG_VERB, "idx:%d, value:%"PRIu32"", now->index, now->value);
    }
}
