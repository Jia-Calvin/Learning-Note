...
static bool
redis_noarg(struct msg *r)
{
    switch (r->type) {
    case MSG_REQ_REDIS_PING:
	case MSG_REQ_REDIS_DPSINFO:
		return true;
    default:
        break;
    }
...
redis_arg0(struct msg *r)
{
    ...
    case MSG_REQ_REDIS_SYNCCACHEDEL:
    case MSG_REQ_REDIS_SYNCSTOREWRITE:

    case MSG_REQ_REDIS_PFCOUNT:

    case MSG_REQ_REDIS_DOUBLECLUSTERMODE:
    ...
    for (p = r->pos; p < b->last; p++) {
        ...
        case SW_REQ_TYPE:
            ...
            case 4:
                ...
                if (str4icmp(m, 'c', 'g', 'e', 't')) {
                    r->type = MSG_REQ_REDIS_CGET;
                    break;
                }

                if (str4icmp(m, 'c', 's', 'e', 't')) {
                    r->type = MSG_REQ_REDIS_CSET;
                    break;
                }
            ...
            case 5:
                ...
                if (str5icmp(m, 'c', 'm', 'g', 'e', 't')) {
                    r->type = MSG_REQ_REDIS_CMGET;
                    break;
                }

                if (str5icmp(m, 'c', 'i', 'n', 'c', 'r')) {
                    r->type = MSG_REQ_REDIS_CINCR;
                    break;
                }

                if (str5icmp(m, 'c', 'd', 'e', 'c', 'r')) {
                    r->type = MSG_REQ_REDIS_CDECR;
                    break;
                }
            ...
            case 7:
				if (str7icmp(m, 'd', 'p', 's', 'a', 'u', 't', 'h')) {
					r->type = MSG_REQ_REDIS_DPSAUTH;
					break;
				}
				...
				if (str7icmp(m, 'd', 'p', 's', 'i', 'n', 'f', 'o')) {
					r->type = MSG_REQ_REDIS_DPSINFO;
					break;
				}

                if (str7icmp(m, 'c', 'i', 'n', 'c', 'r', 'b', 'y')) {
                    r->type = MSG_REQ_REDIS_CINCRBY;
                    break;
                }

                if (str7icmp(m, 'c', 'd', 'e', 'c', 'r', 'b', 'y')) {
                    r->type = MSG_REQ_REDIS_CDECRBY;
                    break;
                }
            ...
            case 12:
                ...
                if (str12icmp(m, 's', 'y', 'n', 'c', 'c', 'a', 'c', 'h', 'e', 'd', 'e', 'l')) {
                    r->type = MSG_REQ_REDIS_SYNCCACHEDEL;
                    break;
                }
            ...
            case 13:
                ...
                break;
            case 14:
                if (str14icmp(m, 's', 'y', 'n', 'c', 's', 't', 'o', 'r', 'e', 'w', 'r', 'i', 't', 'e')) {
                    r->type = MSG_REQ_REDIS_SYNCSTOREWRITE;
                    break;
                }
            ...
            case 17:
                ...
                if (str17icmp(m, 'd', 'o', 'u', 'b', 'l', 'e', 'c', 'l', 'u', 's', 't', 'e', 'r', 'm', 'o', 'd', 'e')) {
                    r->type = MSG_REQ_REDIS_DOUBLECLUSTERMODE;
                    break;
                }
...
redis_parse_rsp(struct msg *r)
{
    ...
    if(unwrapper_migrating_response(r) == NC_ERROR){
        log_hexdump(LOG_INFO, b->pos, mbuf_length(b), "parsed bad migrating response %"PRIu64" "
                "res %d type %d state %d", r->id, r->result, r->type,
                r->state);
        return;
    }
...
redis_post_splitcopy(struct msg *r)
{
    ...
    ASSERT(r->request);
    ASSERT(r->type == MSG_REQ_REDIS_MGET || r->type == MSG_REQ_REDIS_DEL || r->type == MSG_REQ_REDIS_CMGET);
...
redis_pre_coalesce(struct msg *r)
{
    ...
    case MSG_RSP_REDIS_MULTIBULK:
        ...
        ASSERT(pr->type == MSG_REQ_REDIS_MGET || pr->type == MSG_REQ_REDIS_CMGET);
...
redis_post_coalesce(struct msg *r)
{
    ...
    case MSG_RSP_REDIS_MULTIBULK:
        /* only redis 'mget' fragmented request sends back multi-bulk reply */
        ASSERT(r->type == MSG_REQ_REDIS_MGET || r->type == MSG_REQ_REDIS_CMGET);
