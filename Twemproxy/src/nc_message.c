...
#define REDIS_ERROR_LOADING_MSG_LEN 49
...
_msg_get(void)
{
    ...
    msg->retry_count = 0;
    msg->migrate_status = RESULT_UNKOWN;
...
struct msg *
msg_get_empty(const char *key, size_t key_len)
{
    struct msg *msg;
    struct mbuf *mbuf;
    int n;
...
msg_recv(struct context *ctx, struct conn *conn)
{
    ...
    do {
        ...
        if (msg->type == MSG_RSP_REDIS_ERROR) {
            struct mbuf *mbuf = STAILQ_FIRST(&msg->mhdr);
            if (NULL != mbuf) {
                long len = mbuf->last - mbuf->start;
                if (REDIS_ERROR_LOADING_MSG_LEN == len &&
                    0 == nc_strncmp(mbuf->start, "-LOADING Redis is loading the dataset in memory\r\n", len)) {
                    return NC_ERROR;
                }
            }
        }
...
msg_type_info_t type_info[] = {
	...
	{MSG_REQ_MC_SYNCCACHEDEL, "synccachedel", MSG_RW_READ},
	...
	/* ping use to check alive so we let it MSG_RW_WRITE to direct to master*/
	{MSG_REQ_REDIS_PING, "ping", MSG_RW_WRITE},
	/* redis commands - keys */
	...
	{MSG_REQ_REDIS_PFADD, "pfadd", MSG_RW_WRITE},
	{MSG_REQ_REDIS_PFCOUNT, "pfcount", MSG_RW_READ},
	{MSG_REQ_REDIS_PFMERGE, "pfmerge", MSG_RW_WRITE},

	{MSG_REQ_REDIS_SYNCCACHEDEL, "synccachedel", MSG_RW_READ},
	{MSG_REQ_REDIS_SYNCSTOREWRITE, "syncstorewrite", MSG_RW_READ},
	{MSG_REQ_REDIS_DOUBLECLUSTERMODE, "doubleclustermode", MSG_RW_READ},
	{MSG_REQ_REDIS_CLUSTER, "cluster", MSG_RW_READ},
	{MSG_REQ_REDIS_ASKING, "asking", MSG_RW_READ},
	{MSG_REQ_REDIS_READONLY, "readonly", MSG_RW_READ},

	{MSG_REQ_REDIS_GEOADD, "geoadd", MSG_RW_WRITE},
	{MSG_REQ_REDIS_GEOHASH, "geohash", MSG_RW_READ},
	{MSG_REQ_REDIS_GEODIST, "geodist", MSG_RW_READ},
	{MSG_REQ_REDIS_GEORADIUS, "georadius", MSG_RW_READ},
	{MSG_REQ_REDIS_GEORADIUSBYMEMBER, "georadiusbymember", MSG_RW_READ},

	{MSG_REQ_REDIS_DPSAUTH, "dpsauth", MSG_RW_UNKNOWN},
	{MSG_REQ_REDIS_DPSINFO, "dpsinfo", MSG_RW_UNKNOWN},

	/* redis crdt counter */
    { MSG_REQ_REDIS_CSET,             "cset",             MSG_RW_WRITE },
    { MSG_REQ_REDIS_CGET,             "cget",             MSG_RW_READ },
    { MSG_REQ_REDIS_CMGET,            "cmget",            MSG_RW_READ },
    { MSG_REQ_REDIS_CINCR,            "cincr",            MSG_RW_WRITE },
    { MSG_REQ_REDIS_CINCRBY,          "cincrby",          MSG_RW_WRITE },
    { MSG_REQ_REDIS_CDECR,            "cdecr",            MSG_RW_WRITE },
    { MSG_REQ_REDIS_CDECRBY,          "cdecrby",          MSG_RW_WRITE },
...
const uint RSP_MSG_TYPE_LEN = sizeof(type_info) / sizeof(type_info[0]);
