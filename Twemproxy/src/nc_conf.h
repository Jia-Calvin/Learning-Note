...
#define CONF_DEFAULT_READ_LOCAL              false
#define CONF_DEFAULT_SYNC_WRITE_QUEUE_NUM    1
...
#define CONF_DEFAULT_SERVER_CONNECTIONS_WR   1
#define CONF_DEFAULT_KETAMA_PORT             11211

struct conf_gdpr {
	gdpr_auth_mode_t mode;
	gdpr_authority_context_t authority_context;
	gdpr_acl_context_t acl_context;
...
struct conf_pool {
    ...
    int                server_connections_wr; /* server_connections_wr: */
...
/* idc info table ip_addr idc_name */
struct idcinfo {
    struct string addr;
    struct string idcname;
};

extern struct idcinfo conf_idcinfos[];
...
char *conf_set_proxy_password(struct conf *cf, struct command *cmd, void *conf);

void conf_server_init(struct conf_server *cs);
rstatus_t conf_server_transform(struct conf_server *cs, struct server *s,
                                uint32_t nslave);
rstatus_t conf_server_update(struct conf_server *cs, struct server *s);
rstatus_t conf_pool_each_transform(void *elem, void *data);

rstatus_t conf_pool_each_diff(void *elem, void *data);
