...
uint32_t 
mbuf_list_length(struct mbuf *first_buf)
{
    struct mbuf * tmp_buf;
    uint32_t length;
    tmp_buf = first_buf;
    length = mbuf_length(tmp_buf);
    while ((tmp_buf = STAILQ_NEXT(tmp_buf, next)) != NULL) {
        length += mbuf_length(tmp_buf);
    }
    return length;
}
...
struct mbuf *mbuf_append(struct mhdr *h, const char *pos, size_t n)
{
    struct mbuf *mbuf = STAILQ_LAST(h, mbuf, next);
    while (n > 0) {
        size_t len = n;
        if (!mbuf || mbuf_full(mbuf)) {
            mbuf = mbuf_get();
            if (!mbuf) {
                return NULL;
            }
        ...
        if (mbuf_size(mbuf) < len) {
            len = mbuf_size(mbuf);
        }
        mbuf_copy(mbuf, (uint8_t*)pos, len);
        pos += len;
        n -= len;
    }
    return mbuf;
...
struct mbuf *mbuf_expand_space_at_pos(struct mhdr *h, const uint8_t *at_pos, size_t n)
{
    struct mbuf *mbuf = STAILQ_LAST(h, mbuf, next);
     
    ASSERT(!mbuf_full(mbuf) && n <= mbuf_size(mbuf));

    ASSERT(at_pos >= mbuf->start && at_pos < mbuf->last);

    uint8_t * pos = mbuf->last;
    for(;pos >= at_pos; pos--){
        *(pos+n) = *pos; 
    }
    mbuf->last += n;
...
struct mbuf *mbuf_delete_space(struct mhdr *h, const uint8_t *del_from, size_t n)
{
    struct mbuf *mbuf = STAILQ_LAST(h, mbuf, next);
    ASSERT(del_from >= mbuf->start && del_from + n < mbuf->last);
    uint8_t * pos = del_from + n;
    for(;pos <= mbuf->last; pos++){
        *(pos-n) = *pos; 
    }
    mbuf->last -= n;
    return mbuf;
...
struct mbuf *mbuf_append_json(struct mhdr *h, const char *pos, size_t n)
{
    struct mbuf *mbuf = STAILQ_LAST(h, mbuf, next);
    size_t i;
    bool escaped = false;
