...
struct server {
    ...
    struct string      runid;         /* redis cluster running id  */
    uint16_t           port;          /* port */
    ...
    bool               trying;        /* trying to connect */
    uint32_t           failure_count; /* # consecutive failures */

    struct array       slave_pool;    /* slaves. only for stub server */
    struct array       slaves;        /* redis cluster slaves, store struct server* */
...
struct server_pool {
    ...
    uint32_t           server_number;        /* server number */
    struct slot_pool   *slots;               /* redis cluster slots */
    uint32_t           ncontinuum;           /* # continuum points */
    uint32_t           nserver_continuum;    /* # servers - live and dead on continuum (const) */
    struct continuum   *continuum;           /* continuum */
    uint32_t           nlive_server;         /* # live server */
    uint32_t           fail_server[1000];    /* fail server idx */
    ...
    int                sync_write_queue_num; /* sync write queue num */
    int                timeout;              /* timeout in usec */
    int                backlog;              /* listen backlog */
    uint32_t           client_connections;   /* maximum # client connection */
                                             /* not used actually */
    uint32_t           server_connections;   /* maximum # server connection */
    uint32_t           server_connections_wr;/* maximum # server connection_wr */
...
struct conn *server_pool_redirect_conn(struct context *ctx,
...
rstatus_t server_pool_reload(struct array *server_pool, struct array *conf_pool, struct context *ctx);
rstatus_t server_array_update(struct array *server, struct array *conf_server, struct server_pool *sp);
rstatus_t server_conf_reload(struct context *ctx, struct server_pool *pool);
void server_dump(struct server_pool *pool);
void server_failure(struct context *ctx, struct server *server);

extern int reload_config; /* reload config file or not */
extern int double_read; /* cache expand, begin double read */
extern int double_conf; /* cache expand, begin double conf */
#endif
