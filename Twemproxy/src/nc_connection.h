...
struct conn {
	...
	int auth_state;					  /* gdpr auth state */
    ...
    int                write_queue_no; /* write_queue_no */
    ...
    unsigned           readonly_slave:1; /*redis cluster slave node enable read? */
};
...
struct conn * conn_get_manager(void *owner);
