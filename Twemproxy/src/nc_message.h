...
typedef enum msg_type {
	...
	MSG_REQ_MC_SYNCCACHEDEL, /* memcache synccachedel request */
	...
	MSG_REQ_REDIS_SYNCCACHEDEL,
	MSG_REQ_REDIS_SYNCSTOREWRITE,
	MSG_REQ_REDIS_DOUBLECLUSTERMODE,
	MSG_REQ_REDIS_CLUSTER,
	MSG_REQ_REDIS_ASKING,
	MSG_REQ_REDIS_READONLY,
	MSG_REQ_REDIS_GEOADD, /* redis geo */
	MSG_REQ_REDIS_GEOHASH,
	MSG_REQ_REDIS_GEODIST,
	MSG_REQ_REDIS_GEORADIUS,
	MSG_REQ_REDIS_GEORADIUSBYMEMBER,
	MSG_REQ_REDIS_DPSAUTH, /* dpsauth */
	MSG_REQ_REDIS_DPSINFO, /* dpsinfo */
	MSG_REQ_REDIS_CSET, /* redis crdt counter */
	MSG_REQ_REDIS_CGET,
	MSG_REQ_REDIS_CMGET,
	MSG_REQ_REDIS_CINCR,
	MSG_REQ_REDIS_CINCRBY,
	MSG_REQ_REDIS_CDECR,
	MSG_REQ_REDIS_CDECRBY,
...
typedef enum req_redirect_type_t {
    FORCE_REDIRECT,
    MOVED_REDIRECT,
    ASK_REDIRECT
} req_redirect_type_t;

typedef enum rsp_migrate_status {
    RESULT_UNKOWN,
    RESULT_MOVED,
    RESULT_RETRY,
    RESULT_OK
} rsp_migrate_status_t;
...
struct msg {
	...
	rsp_migrate_status_t migrate_status;  /* migrating response status */
	int retry_count; /* migrating response retry retry count */
    ...
    unsigned             miss:1;          /* rsp from get, miss? */
    ...
    uint8_t              nredirect;       /* redirect count */
    ...
    unsigned             swallow:1;       /* swallow response 不用返回给客户端的请求 */
    unsigned             redis:1;         /* redis? */
    unsigned             sending:1;       /* sending? */
    unsigned             inside:1;        /* inside request like sync_cache_del msg, cluster nodes, asking...*/
    unsigned             migrate_wrapper:1; /* commands wrappered in migrating model*/
};
...
extern msg_type_info_t type_info[];
extern const uint RSP_MSG_TYPE_LEN;
...
struct msg *msg_get_empty(const char *key, size_t key_len);
