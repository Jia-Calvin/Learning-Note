...
size_t chromium_urlbase64_encode(char *dest, const char *str, size_t len);
...
size_t chromium_urlbase64_decode(char *dest, const char *src, size_t len);
...
inline std::string &chromium_base64_encode(std::string &s)
{
	...
	size_t d = chromium_base64_encode(const_cast<char *>(x.data()),
					  s.data(), (int)s.size());
	if (d == MODP_B64_ERROR) {
		x.clear();
	} else {
...
inline std::string &chromium_urlbase64_encode(std::string &s)
{
	std::string x(chromium_base64_encode_len(s.size()), '\0');
	size_t d = chromium_urlbase64_encode(const_cast<char *>(x.data()),
					     s.data(), (int)s.size());
	if (d == MODP_B64_ERROR) {
		x.clear();
	} else {
