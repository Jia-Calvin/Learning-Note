...
// 根据前序遍历和中序遍历，重建二叉树
BinaryNodeTree *rebuildBinaryTree(std::vector<int> preOrder,
                                  std::vector<int> middleOrder) {
  ...
  root->_leftNode = rebuildBinaryTree(
      std::vector<int>(preOrder.begin() + 1,
                       preOrder.begin() + leftTreeNum + 1),
  ...
  root->_rightNode = rebuildBinaryTree(
      ...
      std::vector<int>(middleOrder.begin() + leftTreeNum + 1,
                       middleOrder.end()));
