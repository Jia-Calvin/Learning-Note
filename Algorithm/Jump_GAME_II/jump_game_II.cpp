...
// 去到最远的那个点决定， 例如 [2]中的2决定了[1, 3]的3为截止点，[1,3]中的3决定了[1, 3, 1]的1为截止点，
// [1, 3, 1]中的3决定了[4, 1]中的1为截止点，[4, 1]中的4决定了[1, 4    ]的  为截止点（此时大于长度总和结束跳跃）。
// 算法的思想是每次都需要获得下一个区间中能够去到最远的地方。

int jump(vector<int>& nums) {
    int step = 0, curr_max_pos = 0, next_max_pos = 0;
    for (int i = 0; i < nums.size(); i++) {
        // 当前能够到达最远的位置的indx大于等长度-1，则表明能够达到终点，此时跳出
        if (curr_max_pos >= nums.size() - 1) break;
